...
class BaseUnet2D(nn.Module):
    ...
    def forward(self, inputs):
        ...
        up1,up2,up3 = self.decoder(center, conv4, conv3, conv2, conv1)

        final = self.final(up1)
        return {"seg_final": final,"up1":up1,"up2":up2,"up3":up3}
...
class ProjUnet2D(BaseUnet2D):
    def __init__(self, feature_scale=2, n_classes=1, is_deconv=True, in_channels=3, is_batchnorm=True, cfg=None,):
        ...
        if cfg.MODEL.PROJECT_NUM == 5:
            ...
            self.proj_f2 = nn.Sequential(
                unetConv2(self.filters[1], self.filters[2], self.is_batchnorm, n=2, ks=4, stride=4, padding=0)
                 ,unetConv2(self.filters[2], self.filters[3], self.is_batchnorm, n=1, ks=2, stride=2, padding=0),
            )
            self.proj_f3 = nn.Sequential(
                unetConv2(self.filters[2], self.filters[2], self.is_batchnorm, n=2, ks=4, stride=4, padding=0)
                 ,unetConv2(self.filters[2], self.filters[3], self.is_batchnorm, n=1, ks=1, stride=1, padding=0),
            )
        else:
    ...
    def forward(self, inputs, output_final_feat=False):
        ...
        if output_final_feat:
            ...
            rt["proj_f2"] = self.proj_f2(up2)

            rt["proj_f3"] = self.proj_f3(up3)
