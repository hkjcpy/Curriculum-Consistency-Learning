    ...
    for epoch in range(start_epoch, cfg.TRAIN.EPOCHS):
        ...
        for i, batch in enumerate(tbar):
            ...
            if epoch < cfg.OPT.WARM_UP + 1:
                ...
                feature_2 = outputs["proj_f2"]
                feature_3 = outputs['proj_f3']
                p_output = model(p_image, output_final_feat=True)
                p_proj_final = p_output["proj_final"]
                p_output_target = p_output['seg_final']
                p_feature_2 = p_output["proj_f2"]
                p_feature_3 = p_output["proj_f3"]
                #200 64 4 4 ===> 8 20 20 64 1 1 ===> 9
                ...
                p_feature_2 = p_feature_2.reshape(bs, cn, cn, 512 // cfg.MODEL.FEATURE_SCALE, 1, 1).permute(
                    0, 3, 1, 4, 2, 5).reshape(bs, 512 // cfg.MODEL.FEATURE_SCALE, cn, cn)
                p_feature_3 = p_feature_3.reshape(bs, cn, cn, 512 // cfg.MODEL.FEATURE_SCALE, 1, 1).permute(
                    0, 3, 1, 4, 2, 5).reshape(bs, 512 // cfg.MODEL.FEATURE_SCALE, cn, cn)    
            else:
                if epoch < 150: cn = 2
                elif epoch < 200: cn = 4
                ...
                p_image = p_image.unfold(2, cs, cs).unfold(3, cs, cs).permute(
                0, 2, 3, 1, 4, 5).reshape(-1, cfg.DATA.INP_CHANNELS, cs, cs)
                outputs = model(image, output_final_feat=False)
                output_target = outputs['seg_final']
                p_output = model(p_image, output_final_feat=False)
                p_output_target = p_output['seg_final']
            ...
            if epoch < cfg.OPT.WARM_UP:
                ...
                global_cont_loss = 0.5 * cont_loss(proj_final, p_proj_final) + 0.3 * cont_loss(feature_2,p_feature_2) + 0.1* cont_loss(feature_3,p_feature_3)
            else:
            ...
            if epoch < cfg.OPT.WARM_UP:
                ...
                loss = consistency_weight * 0.1 * global_cont_loss + loss_sup
            else:
                consistency_weight = cfg.OPT.MAX_C
